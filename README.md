...
- **Turn, pan and zoom.** Picking the thumbnail hands the gallery's main image over to a viewer. It rotates on its own until the shopper takes hold of it, and then stops getting in their way. By default the view drifts around a model that stays put, and a drag swings the view the same way, so the shadow appears to travel round with the model; a **Spin the model itself** setting instead turns the model on the spot - idling or dragged sideways - while its shadow stays anchored to the floor beneath, its shape following the turning model. Dragging up and down still tilts the view.
- **Follows the shopper's choice.** The product's own model shows while nothing more specific is on offer; once a variation carrying its own is picked, that one takes the strip over. A variation's model otherwise stays out of sight until it is chosen, so a shopper who has picked nothing is not left looking at oak, walnut and ash at once.
